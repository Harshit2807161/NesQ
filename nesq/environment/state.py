...
class CircuitStateDQN:
    ...
    def __init__(self, circuit: CircuitRepDQN, device: DeviceTopology, node_to_qubit=None,
        ...
        self._circuit_progress = np.zeros(len(self.circuit), dtype=np.int64) \
            if circuit_progress is None else circuit_progress
        self._locked_edges = np.zeros(len(self.device.edges), dtype=np.int64) \
    ...
    def target_nodes(self):
        ...
        """
        qubit_to_node = np.zeros(len(self._node_to_qubit), dtype=np.int64)
    ...
    def target_distance(self):
        ...
        for i, v in enumerate(self._node_to_qubit):
            qubit_to_node[v] = i
        target_distances = np.zeros(len(self._node_to_qubit), dtype=np.int64)
        for i, v in enumerate(self._qubit_targets):
    ...
    def remaining_targets(self):
        ...
        for i, v in enumerate(self._node_to_qubit):
            qubit_to_node[v] = i
        remaining_targets = np.zeros(len(self._node_to_qubit), dtype=np.int64)
