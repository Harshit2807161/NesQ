...
def test_cnot_execution():
    cirq_circuit = nesq.environment.circuits.circuit_generated_full_layer(4, 1)
    circuit = nesq.environment.circuits.CircuitRepDQN(cirq_circuit)
    device = nesq.environment.device.GridComputerDevice(2, 2)
    # Testing complete execution
    state = nesq.environment.state.CircuitStateDQN(circuit, device, node_to_qubit=np.arange(4))
    ...
    state = nesq.environment.state.CircuitStateDQN(circuit, device, node_to_qubit=np.array([0, 3, 2, 1]))
    gates_executed = state.execute_cnot()
